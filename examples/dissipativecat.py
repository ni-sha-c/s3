...
def step(u, s=zeros(4), n=1):
    ...
    for i in range(n):
        ...
        psi_0 = sin(2*pi*(vx*x + vy*y))/mag
        psi_1 = sin(2*pi*(vy*x - vx*y))/mag

        u_trj[i+1,0] = (2*x + y + s[0]*psi_1*vy + \
                s[1]*psi_1*vx + s[2]*psi_0*vy + \
                s[3]*psi_0*vx) % 1
        u_trj[i+1,1] = (x + y - s[0]*psi_1*vx + \
                s[1]*psi_1*vy - s[2]*psi_0*vx + \
                s[3]*psi_0*vy) % 1
    return u_trj
def dstep(u, s=[0.,0.]):
    """
